...
void init()
   {
      ...
      Tcl_CreateObjCommand(interp, "alg_division",(Tcl_ObjCmdProc *)alg_division,NULL, NULL);
      //initialise history functions - Begin a session in which the history functions might be used.//
      using_history();
   }

//1.command: alg_division      F                 P         //
//2.                           |                 |         //  
//3.                           |                 |         //
//4.         alg_division {f1 f2 f3...fn}      {p1 p2 p3}  //

//1.the proccess in order to find the quotient               //
//2.do the division of f1 with each pi save the result in a  //
//3.cube_list eg. v11 will be f1/p1 + f2/p1 + .... f1/pn     //
//4.we repeat the same proccess for f2 and we finally    //
//5.have vp1......vpn, and the quotient will be the intersect     
//6. of all vi                                                       //
//7.first we do v = vp1 intersect vp2, then v = v intersect vp3      //
//8.and if we continue the same process by arrive to the end of vpi  //
//9.we will have the intersect of all vpi                            //
int alg_division(ClientData line, Tcl_Interp *interp, int objc, Tcl_Obj *CONST objv[])
   {
      cubes_list* divident;
      cubes_list* divisor; 
      cubes_list* v1; 
      cubes_list* current_divident;
      cubes_list* current_divisor;
      cubes_list* current_vi;
      cubes_list* current_quotient;
      cubes_list* vi;                  //f1/pi + f2/pi + f3/pi + ... fn/pi //
      cubes_list* quotient_tmp;
       cubes_list* quotient;
      char* vi_entry;
      char* quotient_entry;
       char* quotient_entry_reject;
      char* input;
      int i = 0;

       //initialise divident and divisor lists. //

      divident = (cubes_list*)malloc(sizeof(cubes_list));
      divident->prev = NULL;
      divident->next = NULL;
     
      divisor = (cubes_list*)malloc(sizeof(cubes_list));
      divisor->prev = NULL;
      divisor->next = NULL;

      vi = (cubes_list*)malloc(sizeof(cubes_list));
      vi->prev = NULL;
      vi->next = NULL;

      quotient_tmp = (cubes_list*)malloc(sizeof(cubes_list));
      quotient_tmp->prev = NULL;
      quotient_tmp->next = NULL;

      quotient = (cubes_list*)malloc(sizeof(cubes_list));
      quotient->prev = NULL;
      quotient->next = NULL;
       //by we arrive to fn. //
      ...
      while (input != NULL)
         {
            add_list(&divisor,input);
            input = strtok(NULL, " ");
         }
      //the proccess in order to find the quotient. //
      current_divident = divident->next;
      vi_entry = (char*)malloc(strlen(current_divident->cube)*sizeof(char)+1);
      quotient_entry = (char*)malloc(strlen(current_divident->cube)*sizeof(char)+1);
      quotient_entry_reject = (char*)malloc(strlen(current_divident->cube)*sizeof(char)+1);
      for (i = 0; i < strlen(current_divident->cube) ;i++)
         {
            quotient_entry_reject[i] = '1';
         }
      quotient_entry_reject[i] = '\0';
   
       for (current_vi = vi->next; current_vi != NULL; current_vi = current_vi->next)
         {
            printf("%s\n",current_vi->cube);
         }
      for (current_vi = vi->next; current_vi != NULL; current_vi = current_vi->next)
      ...
      for (current_divisor = divisor->next; current_divisor != NULL; current_divisor = current_divisor->next)
         {
            for (current_divident = divident->next; current_divident != NULL; current_divident = current_divident->next)
               {
                  for (i = 0; i < strlen(current_divident->cube) ;i+=2)
                     {
                        //if we have ij/11 the quotient will be ij. //
                        if ((current_divisor->cube)[i] == '1' && (current_divisor->cube)[i+1] == '1') 
                           {
                              vi_entry[i] = (current_divident->cube)[i];
                              vi_entry[i+1] = (current_divident->cube)[i+1];
                           }
                        //if the numerator isn't equal to denominator(eg. a/a' or 11/10) then there is no division between the cubes. //
                        else if ((current_divisor->cube)[i] != (current_divident->cube)[i] || (current_divisor->cube)[i+1] != (current_divident->cube)[i+1]) 
                           {
                              //with i = 0, we declare that there is no division, therefore we don't add to list a new cube. //
                              i = 0; 
                              break;
                           }
                        // if we have a/a the result will be 1(dc) if we have 10/10 the result will be 11. // 
                        else 
                           {
                              vi_entry[i] = '1';
                              vi_entry[i+1] = '1';
                  ...
                  if (i != 0)
                     {
                        add_list(&vi,vi_entry);
                     }
                  
               }
               
               //every time we do f/pi, then we do q(quotient) = previous quotient intersect f/pi(vi) //
               if (current_divisor == divisor->next)   //initialize quotient //
                  {
                     for (current_vi = vi->next; current_vi != NULL ;current_vi = current_vi->next)
                        {
                           add_list(&quotient_tmp, current_vi->cube);
                        }
                  }
               else //intersect //
                  {
                     for (current_quotient = quotient_tmp->next; current_quotient != NULL ;current_quotient = current_quotient->next)
                        {

                           for (current_vi = vi->next; current_vi != NULL ;current_vi = current_vi->next)
                              {
                                 //if there is no intersect the quotient_entry becomes 111111...//
                                 for (i = 0; i < strlen(current_quotient->cube) ;i+=2)
                                    {
                                       if (current_quotient->cube[i] == current_vi->cube[i] && current_quotient->cube[i+1] == current_vi->cube[i+1])
                                          {
                                             quotient_entry[i] = current_quotient->cube[i];  
                                             quotient_entry[i+1] = current_quotient->cube[i+1]; 
                                          }
                                       else 
                                          {
                                             quotient_entry[i] = '1';  
                                             quotient_entry[i+1] = '1';
                                          }
                                    }
                                    quotient_entry[i] = '\0';
                                    if (strcmp(quotient_entry, quotient_entry_reject) != 0)
                                       {
                                          add_list(&quotient, quotient_entry);
                                       }
                                 }
                        }
                     //in the next iteration the current quotient wiil be the intersect of quotients until now. //
                     my_free(&quotient_tmp);
                      for (current_quotient = quotient->next; current_quotient != NULL ;current_quotient = current_quotient->next)
                        {
                           add_list(&quotient_tmp, current_quotient->cube);
                        }
                  }
               my_free(&vi);
         }
